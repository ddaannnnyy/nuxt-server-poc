<template>
    <div v-if="currentlyPlaying"
        class="w-full flex flex-col items-center justify-center gap-2 h-full z-[999] relative transition-all duration-1000"
        ...
        <div class="album-art flex items-center justify-center z-20 w-[40vw]">
            <img :src="currentlyPlaying.item.album.images[0]?.url" alt="" @click.prevent="togglePause"
                @touchmove.prevent @drag.prevent
                class="object-contain w-full shadow-accent transition-transform duration-300 origin-center"
                :class="isPlaying ? 'opacity-100' : 'opacity-50'">
        </div>
        <div
            class="album-details bg-background/40 rounded-2xl z-20 flex flex-col items-center justify-start w-[40v] gap-1  text-center p-10 transition-all duration-200">
            ...
            <ClientOnly>
                <div class="w-full bg-secondary h-1 dark:bg-gray-700 transition-opacity duration-300">
                    ...
                    <p v-if="isPlaying" class="text-text text-xs">{{
                        millisToMinutesAndSeconds(currentlyPlaying.item.duration_ms,
                            currentlyPlaying.progress_ms) }}</p>
                    <p v-else class="text-text text-xs">Paused</p>
                </div>
            </ClientOnly>
        </div>
        <div class="absolute top-0 left-0 w-full h-full backdrop-blur"></div>
    </div>
